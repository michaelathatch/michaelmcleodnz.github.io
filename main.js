window.onload = function() {
	...
	switch(window.location.hash.substring(1)) {
		...
		case "dickson":
			...
			listenAndDisplay2("input","output");
			break;
		default:
			if(test(window.location.hash.substring(1))) {
...
function listenAndDisplay(listenId, displayId, buttonId) {
	...
	if(button.addEventListener) {
		...
		button.attachEvent("onclick", flip); //OldIE
	}
	function callback() {
		document.getElementById(displayId).innerHTML = renderLine(input.value, {fill:"#201", height:92, reverse:rev});
	}
	function flip() {
		rev = !rev;
		document.getElementById(displayId).innerHTML = renderLine(input.value, {fill:"#201", height:92, reverse:rev});
...
function listenAndDisplay2(listenId, displayId) {
	var input = document.getElementById(listenId);
	var wait = false;
	if(input.addEventListener) {
		input.addEventListener("keyup", edited);
	} else if(input.attachEvent) {
		input.attachEvent("onkeyup", edited); //OldIE
	}
	function edited() {
		if(!wait) {
			wait=true;
	...
	function callback() {
		document.getElementById(displayId).innerHTML = displayMatches(input.value, {fontSize:"1.5em", height:32, highlight:"#e7d070"});
		wait = false;
	}
}